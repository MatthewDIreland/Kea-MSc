...
# This code takes a sample of 100 empty sessions and copies all photos in those sessions into a folder 
# where they can be manually tagged.
...
input_csv2 = sys.argv[2] # SessionData.csv
input_csv3 = sys.argv[3] # AnalysisData.csv (Timelapse output with added columns)
...
analysis_rows = [row for row in analysis_table if row[analysis_c_File].startswith("MaxSessionPhotos_") and row[analysis_c_Animal] == "empty"]
...
random_sample_list_of_session_ids = random.sample(analysis_session_ids, sample_size)
...
for analysis_row in random_sample_of_analysis_rows:
    ...
    for image_file in image_file_names:
        ...
        i += 1
        copy_to_file = empty_session_folder + "\\" + str(i) + "_" + os.path.basename(image_file)
